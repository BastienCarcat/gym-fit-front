'use client'
import { AvatarCircles } from '@/components/ui/avatar-circles'
import { ArrowRight } from 'lucide-react'
import Image from 'next/image'
import { motion } from 'framer-motion'
import { cn } from '@/lib/utils'

export default function CTASection() {
  const avatars = [
    {
      imageUrl: 'https://avatars.githubusercontent.com/u/16860528'
    },
    {
      imageUrl: 'https://avatars.githubusercontent.com/u/16860528'
    },
    {
      imageUrl: 'https://avatars.githubusercontent.com/u/16860528'
    },
    {
      imageUrl: 'https://avatars.githubusercontent.com/u/16860528'
    }
  ]

  const fadeInUp = (delay = 0.2) => ({
    hidden: { opacity: 0, y: 20 },
    show: {
      opacity: 1,
      y: 0,
      transition: {
        duration: 0.3,
        delay: delay
      }
    }
  })

  return (
    <section className="relative mx-auto my-48 max-w-7xl px-4 py-20 sm:px-6 sm:py-28 lg:px-8 lg:py-36">
      <div className="flex items-center justify-between">
        <div className="relative z-20 max-w-2xl">
          <motion.h1
            variants={fadeInUp(0)}
            initial="hidden"
            animate="show"
            className="text-4xl text-gray-900 sm:text-6xl"
          >
            Start building your next
            <br />
            <strong className="font-extrabold"> fitness App</strong> today
          </motion.h1>

          <motion.p
            variants={fadeInUp(0.2)}
            initial="hidden"
            animate="show"
            className="pt-8 text-xl text-gray-900 sm:text-2xl"
          >
            Access to all detailed{' '}
            <strong className="font-bold text-sky-500">gym exercises</strong>{' '}
            and
            <br />
            all the{' '}
            <strong className="font-bold text-sky-500">
              fitness calculators
            </strong>{' '}
            you need <br /> through our powerful API.
          </motion.p>
          <motion.div
            initial="hidden"
            animate="show"
            variants={fadeInUp(0.3)}
            className="mt-8 space-x-4"
          >
            <button className="inline-flex h-12 items-center rounded-lg border border-gray-500 px-6 focus:outline-none focus:ring-2 focus:ring-slate-400 focus:ring-offset-2 focus:ring-offset-slate-50">
              See Documentation
            </button>
            <button className="inline-flex h-12 animate-shimmer items-center justify-center rounded-lg border border-sky-700 bg-[linear-gradient(110deg,#0ea5e9,45%,#7dd3fc,55%,#0ea5e9)] bg-[length:200%_100%] px-6 font-medium text-white transition-colors focus:outline-none focus:ring-2 focus:ring-slate-400 focus:ring-offset-2 focus:ring-offset-slate-50">
              Try for Free
              <ArrowRight className="ml-2" />
            </button>
          </motion.div>
          <motion.div
            initial="hidden"
            animate="show"
            variants={fadeInUp(0.4)}
            className="mt-4 flex items-center space-x-2"
          >
            <p>Used by</p>
            <AvatarCircles avatarUrls={avatars} numPeople={900} />
          </motion.div>
        </div>
        <motion.div
          initial="hidden"
          animate="show"
          variants={fadeInUp(0.1)}
          className="absolute right-0 top-0 h-full w-[80%] overflow-hidden"
        >
          <div className="relative h-full w-full rounded-[2.5rem]">
            <Image
              src="/hero_image.jpg"
              alt="Hero image"
              layout="fill"
              objectFit="cover"
              quality={100}
              className={cn(
                'rounded-[2.5rem]',
                '[mask-image:linear-gradient(to_right,transparent_1%,transparent,#000_60%)]'
              )}
            />
          </div>
        </motion.div>
      </div>
    </section>
  )
}
